    ...
    for kadi in users:
        ...
        with st.container(border=True):
            ...
            if os.path.exists(user_file):
                ...
                c4.write(f"**Hedef:** {hedef} kg")
                c4.write(f"**Verilen:** {verilen:.1f} kg")
...
# --- ANA UYGULAMA (Kullanıcılar İçin) ---
    ...
    with st.sidebar:
        ...
        with st.expander("👤 Kişisel Bilgiler", expanded=False):
            ...
            c1, c2 = st.columns([5,2])
            c1.selectbox("Cinsiyet", ["Erkek", "Kadın"], index=0 if p.get("cinsiyet", "Erkek") == "Erkek" else 1, key="w_cins", on_change=profil_kaydet, args=("cinsiyet", "w_cins"))
            with c2: 
                hizala()
                if st.button("🗑️", key="del_cins", use_container_width=True): profil_sil("cinsiyet"); st.rerun()
            
            c1, c2 = st.columns([5,2])
            c1.date_input("Doğum Tarihi", dt_val, min_value=datetime.date(1940,1,1), key="w_dt", on_change=profil_kaydet, args=("dogum_tarihi", "w_dt"))
            with c2:
                hizala()
                if st.button("🗑️", key="del_dt", use_container_width=True): profil_sil("dogum_tarihi"); st.rerun()
            ...
            c1, c2 = st.columns([5,2])
            c1.number_input("Boy (cm)", 100, 250, int(p.get("boy", 175)), key="w_boy", on_change=profil_kaydet, args=("boy", "w_boy"))
            with c2:
                hizala()
                if st.button("🗑️", key="del_boy", use_container_width=True): profil_sil("boy"); st.rerun()

            c1, c2 = st.columns([5,2])
            c1.number_input("Başlangıç Kilosu (kg)", 30.0, 250.0, float(p.get("baslangic_kilo", 80.0)), step=0.1, key="w_bk", on_change=profil_kaydet, args=("baslangic_kilo", "w_bk"))
            with c2:
                hizala()
        ...
        with st.expander("⚖️ Güncel Kilo Takibi", expanded=True):
            ...
            st.number_input("Güncel Kilo (kg)", 30.0, 250.0, kilo, step=0.1, key="w_gk", on_change=profil_kaydet, args=("kilo", "w_gk"))
        ...
        with st.expander("🎯 Hedeflerim", expanded=False):
            c1, c2 = st.columns([5,2])
            c1.number_input("Hedef Kilo (kg)", 30.0, 200.0, float(p.get("hedef_kilo", 70.0)), step=0.1, key="w_hk", on_change=profil_kaydet, args=("hedef_kilo", "w_hk"))
            with c2:
                hizala()
                if st.button("🗑️", key="del_hk", use_container_width=True): profil_sil("hedef_kilo"); st.rerun()

            c1, c2 = st.columns([5,2])
            c1.number_input("Hedef Süre (Gün)", 1, 730, int(p.get("hedef_gun", 30)), key="w_hg", on_change=profil_kaydet, args=("hedef_gun", "w_hg"))
            with c2:
                hizala()
            ...
            if not gunluk_veri["ogünler"]:
                ...
                for m_idx, ogun in enumerate(gunluk_veri["ogünler"]):
                    with st.container(border=True):
                        ...
                        if c_sil.button("🗑️ Tüm Öğünü Sil", key=f"del_o_{islem_tarihi}_{m_idx}"):
                        ...
                        for k_idx, kalem in enumerate(ogun.get("kalemler", [])):
                            ...
                            if c_item_sil.button("❌", key=f"del_i_{islem_tarihi}_{m_idx}_{k_idx}"):
    ...
    # ==========================================
        ...
        with st.container(border=True):
            ...
            if c_ai_buton.button("🤖 Durumumu Değerlendir", use_container_width=True, type="primary"):
